...
def packages():
    ...
    return render_template("user/packages.html",  packages = user.packages)
...
@user.route("/user/view_package/<int:id>")
@login_required
def view_package(id:int):
    permission_required(["User"])
    p: Package = Package.select().where(Package.id == id).first()
    
    return render_template("user/view_package.html", package = p)